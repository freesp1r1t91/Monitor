...
/**
 * Kafka implementation of MessagePublisher that publishes messages to Kafka
 * topics.
 * This implementation provides reliable message delivery with proper error
 * handling
        ...
        try {
            ...
            if (topicParts.length != 2) {
                throw new IllegalArgumentException(
                        "Invalid topic format. Expected 'aggregateType.eventType', got: " + topic);
            }
            String aggregateType = topicParts[0];
            String eventType = topicParts[1];
            // Map to the actual Kafka topic name
            String kafkaTopic = topicNameMapper.mapToTopicName(aggregateType,
                    eventType);
            ...
            future.whenComplete((result, throwable) -> {
                if (throwable != null) {
                    log.error("Failed to publish message to Kafka topic '{}': {}",
                            kafkaTopic, throwable.getMessage(), throwable);
                } else {
            ...
            log.info("Message sent to Kafka topic '{}' (aggregate: {}, event: {})",
                    kafkaTopic, aggregateType, eventType);
        } catch (Exception e) {
