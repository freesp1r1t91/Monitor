...
/**
 * Event handler for domain events that need to be mapped to integration
 * events.
...
public class DomainEventHandler {
    ...
    /**
     * Handles OrderConfirmedEvent by creating and persisting the corresponding
     * integration event.
     * This method is automatically called by Spring's event system when an
     * OrderConfirmedEvent is published.
     * *
     * 
    ...
    public void handleOrderConfirmed(OrderConfirmedEvent event) {
        ...
        try {// Create integration event
            Order order = orderRepository.findByOrderNumber(event.getOrderNumber()).get();
            //String payload = objectMapper.writeValueAsString(order);
            OrderConfirmedIntegrationEvent integrationEvent = new OrderConfirmedIntegrationEvent(
                    order);
            // Persist to outbox for reliable delivery
            persistToOutbox(integrationEvent, "Order",
                    event.getOrderNumber().value());
     ...
     * This method logs the event for debugging purposes but doesn't create
     * integration events.
     * *
     * 
